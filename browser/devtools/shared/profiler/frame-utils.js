...
/**
 * Should the frame with key frameKey be flattened, given the previous frame
 * key prevFrameKey and options.
 *
 * That is, when flattening recursion,
 *   A > B > B > C
 * is flattened to
 *   A > B > C
 *
 * @param string frameKey
 * @param string prevFrameKey
 * @return boolean
 */
exports.shouldFlatten = function shouldFlatten(frameKey, prevFrameKey, options) {
  return options.flattenRecursion && frameKey === prevFrameKey;
};
...
InflatedFrame.prototype.getFrameKey = function getFrameKey(options) {
  ...
  if (options.isLeaf) {
    ...
    return String(this.category || CATEGORY_OTHER);
  }
