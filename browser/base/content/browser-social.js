...
SocialShare = {
  ...
  populateProviderMenu: function() {
    if (!this.iframe)
      return;
    let providers = Social.providers.filter((p) => p.sharedURL);
...
ToolbarHelper.prototype = {
  ...
  clearPalette: function() {
    Social.providers.forEach((p) => this.removeProviderButton(p.origin));
  },
...
SocialMarks = {
  get nodes() {
    let providers = Social.providers.filter((p) => p.markURL);
