...
class InlineMap
{
    ...
    struct InlineEntry
    {
        ...
        void update(KeyInput&& key, ValueInput&& value) {
            this->key = mozilla::Forward<Key>(key);
            this->value = mozilla::Forward<Value>(value);
        }
  ...
  public:
    ...
    MOZ_MUST_USE bool put(KeyInput&& key, ValueInput&& value) {
        ...
        if (p) {
            p->value() = mozilla::Forward<Value>(value);
            return true;
        }
        return add(p, mozilla::Forward<Key>(key), mozilla::Forward<Value>(value));
    }
