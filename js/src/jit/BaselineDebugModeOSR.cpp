          ...
          case JitFrame_BaselineJS: {
            ...
            if (BaselineDebugModeOSRInfo *info = iter.baselineFrame()->getDebugModeOSRInfo()) {
                ...
                if (icEntry) {
                    ...
                    // We could be in the middle of handling an exception.
                    // This happens since we could have bailed out in place
                    // from Ion after a throw, settling on the pc *after* the
                    // bytecode that threw the exception, which may have no
                    // ICEntry.
