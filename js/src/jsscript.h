...
class Bindings
{
    ...
    uint16_t numBodyLevelLets_;
    uint16_t aliasedBodyLevelLetStart_;
    uint32_t numVars_;
  ...
  public:
     ...
     * bindingArray must have length numArgs+numVars+numBodyLevelLets. Before
     * the temporary storage is release, switchToScriptStorage must be called,
     * providing a pointer into the Binding array stored in script->data.
     */
    ...
    uint32_t numBodyLevelLets() const { return numBodyLevelLets_; }
    uint32_t numBlockScoped() const { return numBlockScoped_; }
    uint32_t numBodyLevelLocals() const { return numVars_ + numBodyLevelLets_; }
    uint32_t numLocals() const { return numVars() + numBodyLevelLets() + numBlockScoped(); }
    uint32_t aliasedBodyLevelLetStart() const { return aliasedBodyLevelLetStart_; }

    // Return the size of the bindingArray.
    uint32_t count() const { return numArgs() + numVars() + numBodyLevelLets(); }
...
class JSScript : public js::gc::BarrieredCell<JSScript>
{
  ...
  public:
    ...
    // Number of fixed slots reserved for body-level lets and vars. This value
    // minus nfixedvars() is the number of body-level lets. Only nonzero for
    // function code.
    size_t nbodyfixed() const {
        return function_ ? bindings.numBodyLevelLocals() : 0;
    }

    // Aliases for clarity when dealing with let slots.
    size_t fixedLetBegin() const {
        return nfixedvars();
    }

    size_t fixedLetEnd() const {
        return nfixed();
    }
