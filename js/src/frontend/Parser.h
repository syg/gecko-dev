...
struct StmtInfoPC : public StmtInfoBase {
    ...
    // 'let' declarations inside switches are tricky because the block id
    // doesn't convey dominance information. Record what index the current
    // case's 'let' declarations start at so we may generate dead zone checks
    // for other cases' declarations.
    //
    // Only valid if type is STMT_SWITCH.
    uint16_t        firstDominatingLetInCase;
...
struct ParseContext : public GenericParseContext
{
  ...
  private:
    ...
    DeclVector      bodyLevelLets_; /* let definitions at body-level */
