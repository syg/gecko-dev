...
class FullParseHandler
{
  ...
  public:
    ...
    void markMaybeUninitializedLetUseInSwitch(ParseNode *pn, Definition *dn,
                                              uint16_t firstDominatingLetSlot)
    {
        MOZ_ASSERT(pn->isUsed());
        if (dn->isLet() && dn->pn_cookie.slot() < firstDominatingLetSlot)
