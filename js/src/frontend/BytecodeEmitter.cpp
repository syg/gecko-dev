...
class BytecodeEmitter::EmitterScope : public Nestable<BytecodeEmitter::EmitterScope>
{
  ...
  public:
    ...
    NameLocation lookup(BytecodeEmitter* bce, JSAtom* name) {
        Maybe<NameLocation> loc = lookupInCache(bce, name);
        if (loc)
            return *loc;
...
bool
BytecodeEmitter::emitInitializeFunctionSpecialName(HandlePropertyName name, JSOp initialOp)
{
...
BytecodeEmitter::emitCStyleFor(ParseNode* pn, EmitterScope* headLexicalEmitterScope)
{
    ...
    // The body may not be executed at all; it needs its own TDZ cache.
    {
        TDZCheckCache tdzCache(this);
        if (!emitTree(forBody))
            return false;
    ...
    for (ParseNode* arg = pn->pn_head; arg != funBody; arg = arg->pn_next, argSlot++) {
        ...
        if (initializer) {
            ...
            if (!emit1(JSOP_POP))
                return false;
            {
                // Initializers may not always execute, so they need their own
                // TDZ caches.
                TDZCheckCache tdzCache(this);
