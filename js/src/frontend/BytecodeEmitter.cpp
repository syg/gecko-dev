...
BytecodeEmitter::emitVarOp(ParseNode* pn, JSOp op)
{
    ...
    if (pn->isDefn()) {
        ...
        if (!computeDefinitionIsAliased(this, pn, &op))
            return false;
    }
...
BytecodeEmitter::isAliasedName(BytecodeEmitter* bceOfDef, ParseNode* pn)
{
    ...
    switch (dn->kind()) {
      ...
      case Definition::ARG:
         ...
         */
        return bceOfDef->script->formalIsAliased(pn->pn_scopecoord.slot());
      case Definition::VAR:
...
bool
BytecodeEmitter::computeDefinitionIsAliased(BytecodeEmitter* bceOfDef, ParseNode* pn, JSOp* op)
{
    Definition* dn = pn->resolve();
        ...
        for (StaticScopeIter<CanGC> ssi(cx, outerScope); !ssi.done(); ssi++) {
            ...
            if (ssi.hasSyntacticDynamicScopeObject()) {
                ...
                if (lookupAliasedName(script, pn->pn_atom->asPropertyName(), &slot, pn)) {
                    ...
                    pn->setOp(op);
                    MOZ_ALWAYS_TRUE(pn->pn_scopecoord.setHops(parser->tokenStream, hops));
                    MOZ_ALWAYS_TRUE(pn->pn_scopecoord.setSlot(parser->tokenStream, slot));
                    return true;
                }
                hops++;
            }
