...
class MOZ_STACK_CLASS TryEmitter
{
  ...
  private:
    ...
    // DontUseControl is used by yield* and the internal try-catch around IteratorClose.
    // These internal uses must:
...
class ForOfLoopControl : public LoopControl
{
  ...
  public:
    ...
    bool emitBeginCodeNeedingIteratorClose(BytecodeEmitter* bce) {
        ...
        numYieldsAtBeginCodeNeedingIterClose_ = bce->yieldAndAwaitOffsetList.length();
