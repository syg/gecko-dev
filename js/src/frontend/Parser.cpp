...
{
    if (vars_.length() + bodyLevelLets_.length() >= LOCALNO_LIMIT) {
...
static void
MarkUsesAsHoistedLet(ParseNode *pn)
{
    ...
    // In ES6, 'let' bindings cannot be accessed until initialized.
    ...
    while ((pnu = *pnup) != nullptr && pnu->pn_blockid >= start) {
        ...
        // the 'let' binding, and should not be called afterwards, as it would
        // erroneously unmark hoisted function definitions and needing TDZ
        // checks, which is currently handled in leaveFunction.
                                       ...
                                       HandlePropertyName name, ParseNode *pn, Definition::Kind kind)
{
    ...
    switch (kind) {
      ...
      case Definition::LET:
        dn->setOp(JSOP_INITLET);
        dn->pn_dflags |= (PND_LET | PND_BOUND);
        JS_ASSERT(dn->pn_cookie.level() == staticLevel); /* see bindLet */
        if (atBodyLevel()) {
            if (!bodyLevelLets_.append(dn))
                return false;
            if (!checkLocalsOverflow(ts))
                return false;
        }

        // In ES6, lets cannot be accessed until initialized. If the
        // definition has existing uses, they need to be marked so that we
        // emit dead zone checks.
        MarkUsesAsHoistedLet(pn);
                                                     ...
                                                     InternalHandle<Bindings*> bindings) const
{
    ...
    JS_ASSERT(vars_.length() + bodyLevelLets_.length() < LOCALNO_LIMIT);

    /*
     * Avoid pathological edge cases by explicitly limiting the total number of
     * bindings to what will fit in a uint32_t.
     */
    if (UINT32_MAX - args_.length() <= vars_.length() + bodyLevelLets_.length())
    ...
    for (size_t i = 0; i < bodyLevelLets_.length(); i++) {
        Definition *dn = bodyLevelLets_[i];
        if (!dn->pn_cookie.set(ts, dn->pn_cookie.level(), vars_.length() + i))
            return false;
    }

    uint32_t count = args_.length() + vars_.length() + bodyLevelLets_.length();
    ...
    AppendPackedBindings(this, bodyLevelLets_, packedBindings + args_.length() + vars_.length());

    return Bindings::initWithTemporaryStorage(cx, bindings, args_.length(), vars_.length(),
                                              bodyLevelLets_.length(), blockScopeDepth,
                                              packedBindings);
}
        ...
        for (AtomDefnRange r = pc->lexdeps->all(); !r.empty(); r.popFront()) {
            ...
            if (dn != outer_dn) {
                if (ParseNode *pnu = dn->dn_uses) {
                    // In ES6, 'let' bindings cannot be accessed until
                                                       ...
                                                       bool bodyLevelHoistedUse)
{
    ...
    for (size_t i = 0; i < lazy->numFreeVariables(); i++) {
        ...
        // In ES6, lets are unaccessible before initialization. If the inner
        // function closes over a placeholder definition, we need to mark the
        // variable as maybe needing a dead zone check when we emit bytecode.
        //
...
Parser<ParseHandler>::noteNameUse(HandlePropertyName name, Node pn)
{
    ...
    if (stmt) {
        if (stmt->type == STMT_WITH) {
            ...
            MOZ_ASSERT(stmt->firstDominatingLetInCase <= stmt->staticBlock().numVariables());
            handler.markMaybeUninitializedLetUseInSwitch(pn, dn, stmt->firstDominatingLetInCase);
        }
    }
...
Parser<FullParseHandler>::bindDestructuringVar(BindData<FullParseHandler> *data, ParseNode *pn)
{
     ...
     */
    if (data->op == JSOP_INITLET)
        pn->setOp(JSOP_INITLET);
                                ...
                                StaticBlockObject *blockObj, VarContext varContext)
{
    ...
    do {
        ...
        if (tokenStream.matchToken(TOK_ASSIGN)) {
            ...
            // Per ES6, 'let' declarations may not be accessed until
            // initialized. So a declaration of the form |let x = x| results in a
            // ReferenceError, as the 'x' on the RHS is accessing the let binding
...
Parser<ParseHandler>::switchStatement()
{
    ...
    while ((tt = tokenStream.getToken()) != TOK_RC) {
        ...
        // In ES6, 'let' bindings canot be accessed until initialized. If
        // there was a 'let' declaration in the case we just parsed, remember
        // the slot starting at which new 'let' bindings will be
        // assigned. Since 'let' bindings from previous cases will not
        // dominate uses in the current case, any such uses will require a
        // dead zone check.
        if (pc->blockNode != caseList)
            stmtInfo.firstDominatingLetInCase = stmtInfo.staticBlock().numVariables();
